...
function [rldata_r, k_break, rlpol, gvec, real_ax_pts] = rlocus_dyn2(sys, ax12, extents)
  ...
  if (nargout  == 0)
    ...
    points = [real(rlpolv), imag(rlpolv) ];
...
##    set (gcf,"visible","off");
    ...
    ax = gca ();
    set (gcf, "windowbuttondownfcn", {@down_fig, ax12,  olpol, olzer});
    set (gcf, "windowbuttonmotionfcn", {@drag_fig, ax12, olpol, olzer});
    set (gcf, "windowbuttonupfcn", {@up_fig, ax12, olpol, olzer});
    set (ax, 'UserData', []);
...
function rlpol = sort_roots (rlpol, tolx, toly)
  ...
  ## reorder entries in each column of rlpol to be by their nearest-neighbors down_fig (hsrc, evt, olpol, olzer)rlpol
  dp = diff (rlpol.').';
  drp = max (real (dp));
  dip = max (imag (dp));
...
function down_fig (hsrc, evt, ax12, olpol, olzer)
  down_fig =1;
...
##  drag_fig (hsrc, evt, olpol, olzer);
   
endfunction

function drag_fig (hsrc, evt, ax12,olpol, olzer)  

endfunction

function up_fig (hsrc, evt, ax12, olpol, olzer)
  ...
  if poles(2, idx) > 0
    p = -1*poles(2, idx);
    idxc = find(poles(2,:) == p, 1, 'last');
    poles(:, idxc) = conj(poles(2, idx));
  endif
