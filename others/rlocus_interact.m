...
function rlocus_eri (sys, increment, min_k, max_k)
  ...
  f = figure('Name','Root Locus Editor','NumberTitle','off');
  rlocus_solve(f,0,0, sys);
end

function rlocus_solve(f, evt, ax, sys)
  ...
  if (nargout  == 0)
    ...
    %set (gcf,"visible","off");
    hplt = plot (args{:});
    set (hplt(kk--), "markersize", 8, "linewidth", 4); #2);
    if (! isempty (rlzer))
      set (hplt(kk--), "markersize", 8, "linewidth", 4); #2);
    endif
    for ii = 1:rows(rlpol)
  ...
  ax = gca ();
  set (f, 'WindowButtonDownFcn', {@onButtonDown, ax, sys});
  set (ax, 'UserData', []);
...
function onButtonDown (f, evt, ax, sys)
  ...
  if (strcmp (st, 'normal')) # LEFT
            
    new_pol = [olpol; pt(1,1)+pt(1,2)*i; pt(1,1)-pt(1,2)*i];
    new_zer = olzer;
    sys = zpk (new_zer, new_pol,[1]);
    new_zer
    new_pol
    set (f, 'WindowButtonDownFcn', {@rlocus_solve, ax, sys});
    
  elseif (strcmp (st, 'extend') || strcmp (st, 'alt'))

    if (strcmp (st, 'alt')) # RIGHT
      
    new_zer = [olzer; pt(1,1)+pt(1,2)*i; pt(1,1)-pt(1,2)*i];
    new_pol = olpol;
    sys = zpk (new_zer, new_pol,[1]);##    
    ...
    elseif (strcmp (st, 'extend')) # CENTER
      ...
      if xp < xr
        if (ixp == 1)
          new_pol = olpol(ixp+1:end)
        elseif ixp == length(olpol)
          new_pol = olpol(1:end-1)
        elseif
          new_pol = [olpol(1:ixp-1);olpol(ixp+1:end)]
        endif
        new_zer = olzer;
      else
        if (ixr == 1)
          new_zer = olzer(ixr+1:end)
        elseif ixr == length(olzer)
          new_zer = olzer(1:end-1)
        elseif
          new_zer = [olzer(1:ixr-1);olzer(ixr+1:end)]
        endif
        new_pol = olpol;
      end
      new_zer
...
function onButtonMotion (f, evt, ax, p1, l, sys)
